...
/// Performance tests for preset system
final class PresetPerformanceTests: XCTestCase {
    ...
    func testThumbnailGenerationDoesntBlockUI() throws {
        // Create a preset
        let transition = TransitionClip(
            type: .wipe,
        ...
        guard let preset = library.allPresets.first(where: { $0.name == "Thumbnail Test" }) else {
